...
import cv2
import os
from neural_style_logic import style_adain
...
def parse_args(commandLine):
    ...
    parser.add_argument('--init_img_type', type=str,
                        default='content',
                        choices=['random', 'content', 'style', 'adain'],
    ...
    parser.add_argument('--model_weights', type=str,
                        default='neural_style_logic/imagenet-vgg-verydeep-19.mat',
    ...
    parser.add_argument('--img_display_output_dir', type=str,
                        default='./media/static/images/',
                        help='Relative or absolute directory path to displayed output image and data.')
...
def write_image_output(output_img, content_img, style_imgs, init_img):
    ...
    out_display_dir = os.path.join(args.img_display_output_dir, args.img_name)
    maybe_make_directory(out_dir)
    img_path = os.path.join(out_dir, args.img_name + '.png')
    display_img_path = os.path.join(out_display_dir, args.img_name + '.png')
    content_path = os.path.join(out_dir, 'content.png')
    init_path = os.path.join(out_dir, 'init.png')

    write_image(img_path, output_img)
    write_image(display_img_path, output_img)
    print("WROTE TO {}".format(display_img_path))
...
def get_init_image(init_type, content_img, style_imgs, frame=None):
    if init_type == 'content':
        ...
        return style_imgs[0]
    elif init_type == 'adain':
        print("~~~~~~~~~~~~~~init with adain~~~~~~~~~~~~~~~~")
        img = style_adain.stylize_single_adain(content_img, style_imgs[0], 'neural_style_logic/vgg19_normalised.npz')
        img = np.resize(img, content_img.shape)
        return img
...
class DefaultRequest():
    def __init__(self,
        ...
        device = "--device cpu",
        verbose = "--verbose",
        init_img_type = "--init_img_type adain",
